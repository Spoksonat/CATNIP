...
class CTSBIWindow(ctk.CTk):

    def __init__(self, controller):
        super().__init__()
        self.controller = controller
        self.protocol("WM_DELETE_WINDOW", self.on_close)
        self.title("CT SBI Options")
    ...
    def setup_tab1(self):
        ...
        self.add_sample_section(parent=tab1, CT=self.CT)
        self.add_DF_section(parent=tab1)
        self.add_geometry_section(parent=tab1, geom_params=geom_params)
        self.add_ct_section(parent=tab1)
    
        for frame in [self.source_frame, self.det_frame, self.grat_frame, self.samp_frame, self.geom_frame, self.DF_frame, self.ct_frame]:
            for c in range(6):
    ...
    def setup_tab2(self):
        ...
        # Run button
        run_button = ctk.CTkButton(btn_run_frame, text="Run CT SBI Simulation", command=self.run_CT_SBI_sim)
        run_button.pack(side="left", padx= 10, pady=(0, 10))

        # Run button
        run2_button = ctk.CTkButton(btn_run_frame, text="Run CT SBI Simulation on Server", command=self.run_CT_SBI_sim_server)
        run2_button.pack(side="left", padx= 10, pady=(0, 10))

        ## Run button
        #retrieve1_button = ctk.CTkButton(btn_run_frame, text="Transmission Retrieval", command=self.go_to_ct_inline_retrieval)
    ...
    def add_sample_section(self, parent, CT):
        add_sample_section_general(self, parent, CT)
    ...
    def add_ct_section(self, parent):
        add_ct_section_general(self, parent)

    def run_CT_SBI_sim(self):
        run_CT_SBI_sim(self)

    def run_CT_SBI_sim_server(self):
        self.define_param_dict()
        ...
        self.destroy()
        self.controller.ext_script = "ct_sbi.py"
        self.controller.show_ct_sbi_window_server()
