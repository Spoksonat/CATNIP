...
class CTEIWindow(ctk.CTk):

    def __init__(self, controller):
        super().__init__()
        self.controller = controller
        self.protocol("WM_DELETE_WINDOW", self.on_close)
        self.title("CT EI Options")
    ...
    def setup_tab1(self):
        ...
        geom_params = ["Source-Detector distance (m)", "Grating-Sample distance (m)", "Shift grating in prop. axis (cm)", "Shift grating lateral axis (μm)"]
        grat_params = ["Period (μm)", "Fringe width (μm)", "Grating thickness (μm)", "Material", "Material density (g/cc)", "Number of steps"]
    ...
    def setup_tab2(self):
        ...
        # Run button
        run_button = ctk.CTkButton(btn_run_frame, text="Run CT EI Simulation", command=self.run_CT_EI_sim)
        run_button.pack(side="left", padx= 10, pady=(0, 10))

        # Run button
        run2_button = ctk.CTkButton(btn_run_frame, text="Run CT EI Simulation on Server", command=self.run_CT_EI_sim_server)
    ...
    def run_CT_EI_sim(self):
        run_CT_EI_sim(self)

    def run_CT_EI_sim_server(self):
        self.define_param_dict()
        ...
        self.destroy()
        self.controller.ext_script = "ct_ei.py"
