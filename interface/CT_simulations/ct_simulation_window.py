...
class CTSimulationsWindow(ctk.CTk):
    def __init__(self, controller):
        ...
        ctk.CTkButton(button_frame, text="Inline", width=100, command=self.open_ct_inline_window).pack(side="left", padx=20)
        ctk.CTkButton(button_frame, text="SBI", width=100, command=self.open_ct_sbi_window).pack(side="left", padx=20)
        ctk.CTkButton(button_frame, text="SGBI", width=100, command=self.open_ct_sgbi_window).pack(side="left", padx=20)
        ctk.CTkButton(button_frame, text="EI", width=100, command=self.open_ct_ei_window).pack(side="left", padx=20)
    ...
    def open_ct_inline_window(self):
        self.destroy()
        self.controller.show_ct_inline_window()

    def open_ct_sbi_window(self):
        self.destroy()
        self.controller.show_ct_sbi_window()

    def open_ct_sgbi_window(self):
        self.destroy()
        self.controller.show_ct_sgbi_window()

    def open_ct_ei_window(self):
        self.destroy()
