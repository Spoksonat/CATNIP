...
class SGBIWindow(ctk.CTk):

    def __init__(self, controller):
        super().__init__()
        self.controller = controller
        self.protocol("WM_DELETE_WINDOW", self.on_close)
        self.title("SGBI Options")
    ...
    def setup_tab1(self):
        ...
        grat_params = ["Period in X (μm)", "Period in Y (μm)", "Thickness (μm)", "Duty cycle", "Phase shift", "Hole vertices", "Material", "Material density (g/cc)", "Num. of steps per dir."]
        ...
        showalg_btn = ctk.CTkButton(btn_frame, text="Show Reference Pattern", command=self.show_reference)
    ...
    def setup_tab2(self):
        ...
        # Run button
        run_button = ctk.CTkButton(btn_run_frame, text="Run SGBI Simulation", command=self.run_SGBI_sim)
        run_button.pack(side="left", padx= 10, pady=(0, 10))

        # Run button
        run2_button = ctk.CTkButton(btn_run_frame, text="Run SGBI Simulation on Server", command=self.run_SGBI_sim_server)
    ...
    def show_setup(self):
        ...
        fig = self.plots.plot_setup_sgbi(dict_params=self.dict_params)
        fig.canvas.manager.set_window_title("Setup")
        plt.show()

    def show_reference(self):
        show_reference_sgbi(self)
    ...
    def run_SGBI_sim(self):
        run_SGBI_sim(self)   

    def run_SGBI_sim_server(self):
        self.define_param_dict()
        ...
        self.destroy()
        self.controller.ext_script = "sgbi.py"
        self.controller.show_sgbi_window_server()
