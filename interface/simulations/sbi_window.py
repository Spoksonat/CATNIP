...
class SBIWindow(ctk.CTk):

    def __init__(self, controller):
        super().__init__()
        self.controller = controller
        self.protocol("WM_DELETE_WINDOW", self.on_close)
        self.title("SBI Options")
    ...
    def setup_tab1(self):
        ...
        geom_params = ["Source-Detector distance (m)", "Source-Grating distance (m)", "Grating-Sample distance (m)"]
        grat_params = ["Grain size (μm)", "Paper thickness (μm)", "Grain material", "Grain material density (g/cc)", "Paper material", "Paper material density (g/cc)", "Number of sandpapers","Number of steps", "Step size (μm)", "Number of grains"]

        self.add_source_section(parent=tab1)
        self.add_det_section(parent=tab1)
        self.add_grat_section(parent=tab1, grat_params=grat_params)
        self.add_sample_section(parent=tab1)
        self.add_DF_section(parent=tab1)
        self.add_geometry_section(parent=tab1, geom_params=geom_params)
    
        for frame in [self.source_frame, self.det_frame, self.grat_frame, self.samp_frame, self.DF_frame, self.geom_frame]:
            for c in range(6):
        ...
        # Show speckle pattern button (right of Save)
        showalg_btn = ctk.CTkButton(btn_frame, text="Show Speckle Pattern", command=self.show_speckles)
        showalg_btn.pack(side="left")
    ...
    def setup_tab2(self):
        ...
        # Run button
        run_button = ctk.CTkButton(btn_run_frame, text="Run SBI Simulation", command=self.run_SBI_sim)
        run_button.pack(side="left", padx= 10, pady=(0, 10))

        # Run button
        run2_button = ctk.CTkButton(btn_run_frame, text="Run SBI Simulation on Server", command=self.run_SBI_sim_server)
        run2_button.pack(side="left", padx= 10, pady=(0, 10))

        # Run button
        retrieve1_button = ctk.CTkButton(btn_run_frame, text="UMPA Retrieval", command=self.go_to_umpa_retrieval)
        retrieve1_button.pack(side="left", padx= 10, pady=(0, 10))

        # Run button
        #retrieve2_button = ctk.CTkButton(btn_run_frame, text="MIST Retrieval", command=self.go_to_mist_retrieval)
        #retrieve2_button.pack(side="left", padx= 10, pady=(0, 10))

        # Run button
        retrieve3_button = ctk.CTkButton(btn_run_frame, text="LCS Retrieval", command=self.go_to_lcs_retrieval)
        retrieve3_button.pack(side="left", padx= 10, pady=(0, 10))
    ...
    def show_setup(self):
        ...
        fig = self.plots.plot_setup_sbi(dict_params=self.dict_params)
        fig.canvas.manager.set_window_title("Setup")
        plt.show()

    def show_speckles(self): 
        show_speckles(self)
    ...
    def add_grat_section(self, parent, grat_params):
        add_grat_section_general(self, parent, grat_params)

    def add_sample_section(self, parent):
        add_sample_section_general(self, parent)

    def add_DF_section(self, parent):
        add_DF_section_general(self, parent)

    def add_geometry_section(self, parent, geom_params):
        add_geometry_section_general(self, parent, geom_params)

    def run_SBI_sim(self):
        run_SBI_sim(self)

    def run_SBI_sim_server(self):
        self.define_param_dict()
        ...
        self.destroy()
        self.controller.ext_script = "sbi.py"
        self.controller.show_sbi_window_server()

    def run_ext_sim(self):
        run_ext_sim_general(self)

    def go_to_umpa_retrieval(self):
        self.controller.flag_sim_to_ret = True
        self.controller.show_umpa_retrieval_window()

    def go_to_mist_retrieval(self):
        self.controller.flag_sim_to_ret = True
        self.controller.show_ei_retrieval_window()

    def go_to_lcs_retrieval(self):
        self.controller.flag_sim_to_ret = True
