...
class GratingSGBI:
    ...
    def __init__(self, dict_params, E: float) -> None:
        ...
        self.dc = float(dict_params["Duty cycle"]) # Duty cycle
        ...
        self.mat = dict_params["Material"] # Material name
        self.mat_density = float(dict_params["Material density (g/cc)"]) # Material density in g/cm3
        self.n_vert = int(float(dict_params["Hole vertices"])) # Number of vertices of 2D grating hole (optional)
        self.bin_grat = None # Grating binary pattern
        self.E = E
    ...
    def create_hole(self) -> np.ndarray:
        ...
        angles = np.linspace(0, 2 * np.pi, self.n_vert, endpoint=False)
        x = self.radius_pix * np.cos(angles)
        y = self.radius_pix * np.sin(angles)
        ...
        if(self.n_vert % 2 == 0):
            ...
            x_rot = np.cos(0.5*np.pi/self.n_vert) * x + np.sin(0.5*np.pi/self.n_vert) * y
            y_rot = -np.sin(0.5*np.pi/self.n_vert) * x + np.cos(0.5*np.pi/self.n_vert) * y

        vertices = np.array([cx + x_rot, cy + y_rot]).T
